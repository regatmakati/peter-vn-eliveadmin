<?php
    ...
    function getUserInfo($uid) {
        $where['id']=$uid;
        $info= Db::name("user")->field("id,user_nicename,avatar,avatar_thumb,sex,signature,consumption,votestotal,province,city,birthday,issuper,subscribenum,viewnum")->where($where)->find();
		if(!$info){
            $info['id']=$uid;
	...
	*/
	function PrivateKey_tx($host,$stream,$type){
		...
		if($type==1){
			...
			if($ext){
				$url = "{$pull}/live/" . $live_code . ".".$ext.$play_safe_url;
			}
...
// pc 统一返回格式
function jsonList($errno = 0, $errmsg = '', $data = [])
{
    return json_encode(['errno' => $errno , 'errmsg' => $errmsg, 'data' => $data],JSON_UNESCAPED_UNICODE);

}
    //获取比赛战队总人数
    function getAllPeople($team_a_id,$team_b_id,$table='lol_player'){
        $count = Db::name($table)
            ->where('team_id', ['=',$team_a_id],['=',$team_b_id],'or')
            ->count();
        return $count;
    }

    //获取体育比赛战队总人数
    function getSportsAllPeople($homeId,$awayId,$table='sports_basketball_player'){
        $count = Db::name($table)
            ->where('teamId', ['=',$homeId],['=',$awayId],'or')
            ->count();
        return $count;
    }
    ...
    {
        $sensitiveWords = json_decode($GLOBALS['redisdb']->get( 'sensitive_words'), true);
	...
	*  @param int $type 类型，0表示播流，1表示推流
	*/
	function PrivateKey_tx_cs($stream,$type,$pull,$play_url_key){
		$configpri=getConfigPri();
		$bizid=$configpri['tx_bizid'];
		$push_url_key=$configpri['tx_push_key'];
		//$play_url_key=$configpri['tx_play_key'];
