...
class AdminIndexController extends AdminBaseController{
	...
	function addPost(){
		if ($this->request->isPost()) {
            ...
            if (checkSensitiveWords($data['user_nicename'])) $this->error("用户名不能包含违禁词！");
            if (checkSensitiveWords($data['signature'])) $this->error("签名不能包含违禁词！");
	...
	function UserRedis(){
        $users = Db::name("user")
            ->field('id,user_nicename,avatar,avatar_thumb,user_status')
            ->where("user_type=2")
            ->select();
        foreach ($users as $k=>$v){
            setcaches("chatUserinfo_".$v['id'],$v);
        }


        $usertokens = Db::name("user_token")
            ->field('user_id,token,expire_time')
            ->where("expire_time>=".time())
            ->select();
        $redis = connectionRedis();
        foreach ($usertokens as $k=>$v){
            $redis->setex("chatToken_" . $v['user_id'], $v['expire_time'] - time(), $v['token']);
        }
        echo 'ok';
    }
